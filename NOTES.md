...
purpose: "JS-first index for DS & algorithms; ties ZTM cheatsheets to this repo. Primary focus: runnable JS in JS/."
category: "index"
...
JS-first notes tied to **Zero to Mastery cheatsheets** and this repo. **Start with the `JS/` folder**; use TypeScript and `TS_file/` once you're comfortable with the logic in JavaScript.
...
- **New notes or folder changes** (e.g. splitting into `notes/JS/`, `notes/BigO.md`) do require a plan and your approval first (see `.cursor/rules/obsidian-java-notes.mdc`).
...
All runnable examples live in **`JS/`**. Run with Node (no build step).

## Run commands

```bash
# Run one example
node JS/01-sameFrequency.js
...
| #   | JS file                      | Topic                                    | See also (TS / repo)                              |
| --- | ---------------------------- | ---------------------------------------- | ------------------------------------------------- |
| 01  | `JS/01-sameFrequency.js`     | Same digit frequency between two numbers | `TS_file/sameFrequency.ts`                        |
| 02  | `JS/02-recursion.js`         | Countdown, factorial, collect odds       | `TS_file/recursion.ts`, `sd421/D4_Recursion.ts`   |
| 03  | `JS/03-classes.js`           | Student, Point (instance + static)       | `TS_file/classRevision.ts`                        |
| 04  | `JS/04-singleLinkedList.js`  | SLL: push, pop, get, reverse             | `TS_file/singleLinkedList.ts`, `sd421/D2_DLL.ts`  |
| 05  | `JS/05-doubleLinkedList.js`  | DLL: push, pop, shift, unshift, get      | `TS_file/DoubleLinkedList.ts`                     |
| 06  | `JS/06-stack.js`             | Stack (LIFO)                             | `TS_file/stack.ts`, `sd421/D3_StackNQueue.ts`     |
| 07  | `JS/07-queue.js`             | Queue (FIFO)                             | `TS_file/queue.ts`                                |
| 08  | `JS/08-binarySearchTree.js`  | BST: insert, find, BFS, DFS              | `TS_file/binarySearchTree.ts`, `sd421/D5_Tree.ts` |
| 09  | `JS/09-maxBinaryHeap.js`     | Max heap: insert, extractMax             | `TS_file/maxBinaryHeap.ts`, `sd421/D6_Heap.ts`    |
| 10  | `JS/10-priorityQueue.js`     | Min-heap priority queue                  | `TS_file/priorityQueue.ts`                        |
| 11  | `JS/11-hashTable.js`         | Hash table: set, get, keys, values       | `TS_file/hashTable.ts`, `sd421_2/D1_Hash.ts`      |
| 12  | `JS/12-graph.js`             | Graph: addVertex, addEdge, DFT, BFT      | `TS_file/graph.ts`, `sd421_2/D5_DFS_BFS.ts`       |
| 13  | `JS/13-bigO.js`              | Count evens, O(1) / O(n) / O(n²)         | `sd421/d1_BigO.ts`                                |
| 14  | `JS/14-sequenceADT.js`       | Remove duplicate, isPermutation          | `sd421/D2_Sequence_ADT.ts`                        |
| 15  | `JS/15-recursionAdvanced.js` | Fibonacci, isEven, power                 | `sd421/D4_Recursion.ts`                           |
| 16  | `JS/16-examHelpers.js`       | isEvenMutual, sum, sumFirst              | `sd421/midExam.ts`                                |

**Sorting / searching:** No dedicated JS file in `JS/`; see `TS_file/sort.ts`, `TS_file/searchingAlgo.ts`, `sd421/D7_SelectionSort.ts`, `sd421/D8_RadixNBucketSort.ts`.

## Suggested learning order (JS-first)

1. **Basics:** `01` → `02` → `03` (frequency, recursion, classes).
2. **Linear structures:** `04` → `05` → `06` → `07` (SLL, DLL, stack, queue).
3. **Trees & heaps:** `08` → `09` → `10` (BST, max heap, priority queue).
4. **Hash & graph:** `11` → `12` (hash table, graph DFT/BFT).
5. **Complexity & sequences:** `13` → `14` (Big O examples, sequence ADT).
6. **Recursion & exam-style:** `15` → `16` (fibonacci, power, isEven, sum helpers).

---

# Part 2 — Big O (use with JS/13-bigO.js)
...
- **Run:** `node JS/13-bigO.js` (count evens, O(1)/O(n)/O(n²) examples).
...
**Run:** No dedicated file in `JS/`; see `ZTM/Arrays/` (array.ts, moveZeroes, reverseStr).
...
**Run:** `node JS/11-hashTable.js` · **See also:** `TS_file/hashTable.ts`, `sd421_2/D1_Hash.ts`
...
**Run:** `node JS/04-singleLinkedList.js` · `node JS/05-doubleLinkedList.js` · **See also:** `TS_file/singleLinkedList.ts`, `TS_file/DoubleLinkedList.ts`, `sd421/D2_DLL.ts`
...
**Run:** `node JS/06-stack.js` · **See also:** `TS_file/stack.ts`, `sd421/D3_StackNQueue.ts`
...
**Run:** `node JS/07-queue.js` · **See also:** `TS_file/queue.ts`
...
**Run:** `node JS/08-binarySearchTree.js` · **See also:** `TS_file/binarySearchTree.ts`, `sd421/D5_Tree.ts`, `sd421_2/AVL.ts`
...
**Run:** `node JS/09-maxBinaryHeap.js` · **See also:** `TS_file/maxBinaryHeap.ts`, `sd421/D6_Heap.ts`
...
**Run:** `node JS/10-priorityQueue.js` · **See also:** `TS_file/priorityQueue.ts`
...
**Run:** `node JS/12-graph.js` · **See also:** `TS_file/graph.ts`, `sd421_2/D5_DFS_BFS.ts`
...
**Run:** `node JS/02-recursion.js` · `node JS/15-recursionAdvanced.js` · `node JS/16-examHelpers.js` · **See also:** `TS_file/recursion.ts`, `sd421/D4_Recursion.ts`, `sd421/midExam.ts`
...
**See also:** `TS_file/sort.ts`, `sd421/D7_SelectionSort.ts`, `sd421/D8_RadixNBucketSort.ts` (no JS file in `JS/`).
...
**Run:** `node JS/12-graph.js` (BFT, DFT) · **See also:** `TS_file/graph.ts`, `TS_file/treeTraversal.ts`, `sd421_2/D5_DFS_BFS.ts`
...
1. **Run all JS examples** — `node JS/run-all.js`.
2. **One JS file per day** — Pick one row from the JS quick reference; run the file, read it, then re-type it without looking.
3. **Big O drill** — Open `JS/13-bigO.js`; for each function, label O(1), O(n), or O(n²) and say why.
...
This note is your **index/MOC** and JS-first learning path; keep it as the single entry point and link to `JS/` first, then TS and repo.
